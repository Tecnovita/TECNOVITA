  ...
  return (
    <>
      ...
      >
        <div className="w-full flex items-center justify-between h-16 md:h-20 px-4 md:px-8 lg:px-12">
          ...
          <div className="flex items-center shrink-0">
            <Link className="inline-flex items-center group relative" href="/">
              <div className="relative w-[120px] md:w-[160px] lg:w-[200px] max-h-[56px] md:max-h-[72px]">
                <div className="absolute inset-0 bg-blue-500/20 blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                <Image priority alt="Logo de TECNOVITA" className="relative w-full h-auto object-contain transition-all duration-300 group-hover:scale-105 group-hover:drop-shadow-[0_0_15px_rgba(59,130,246,0.5)]" height={200} src="/logo.svg" width={800} />
              </div>
            </Link>
          </div>

          {/* 2. REDES SOCIALES (Centro - Restaurado) */}
            ...
            {socialLinks.map((social) => {
              ...
              return (
                ...
                >
                  ...
                  <Icon className={`text-xl md:text-2xl lg:text-3xl transition-all duration-300 relative z-10 ${social.color} ${social.glowColor}`} />
                </Link>
              );
            })}
          </div>
          ...
          <div className="flex items-center gap-3">
            <nav className="hidden md:flex items-center gap-6 lg:gap-8 text-base lg:text-lg font-semibold">
              <Link className="relative text-blue-400 hover:text-blue-300 transition-colors group py-2" href="/">
              ...
              {/* Dropdown Automatizado */}
              <div className="relative group py-2">
                <button className="flex items-center gap-1 text-blue-400 hover:text-blue-300 transition-colors">
                  Servicios <FaChevronDown className="text-xs transition-transform duration-300 group-hover:rotate-180" />
                </button>
                <div className="absolute top-full right-0 mt-1 w-56 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
                  <div className="bg-black/95 backdrop-blur-xl border border-blue-500/20 rounded-xl shadow-2xl overflow-hidden py-2">
                    {services.map((s) => (
                      <Link key={s.id} className="flex items-center gap-3 px-4 py-3 text-sm text-blue-400 hover:bg-blue-500/10 hover:text-blue-300 transition-colors" href={`/servicios/${s.id}`}>
              ...
              <Link className="relative text-blue-400 hover:text-blue-300 transition-colors group py-2" href="/contacto">
            ...
            {/* Menú Móvil */}
            <button className={`md:hidden p-3 text-2xl rounded-xl transition-all duration-300 ${isOpen ? 'bg-blue-500/20 text-blue-400 rotate-90' : 'bg-gray-800/50 text-blue-400 hover:bg-gray-700/50'}`} onClick={toggleMenu}>
        ...
        <div className={`md:hidden w-full bg-gradient-to-b from-black via-gray-900 to-black transition-all duration-500 overflow-hidden border-t ${isOpen ? 'max-h-[500px] border-blue-500/30' : 'max-h-0 border-transparent'}`}>
          <nav className="flex flex-col py-3">
            <Link className="block px-6 py-4 text-blue-400 hover:bg-blue-500/10 font-semibold" href="/" onClick={() => setIsOpen(false)}>Inicio</Link>
            <div className="px-6 py-2 text-xs font-bold text-gray-500 uppercase">Servicios</div>
            {services.map((s) => (
              <Link key={s.id} className="block px-8 py-3 text-blue-400 hover:bg-blue-500/10 font-medium" href={`/servicios/${s.id}`} onClick={() => setIsOpen(false)}>
                <div className="flex items-center gap-3"><s.icon />{s.id.charAt(0).toUpperCase() + s.id.slice(1)}</div>
              </Link>
            ))}
            <Link className="block px-6 py-4 text-blue-400 hover:bg-blue-500/10 font-semibold mt-2" href="/contacto" onClick={() => setIsOpen(false)}>Contacto</Link>
          </nav>
        </div>
      </header>
